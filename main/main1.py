...
def join_csv(csv_list, dset_name):
    ...
    df = fit.transform(df)
    print(df.shape)
    # Reshape target to match the number of columns in the feature matrix
    target = target.reshape(-1, 1)
    # Concatenate feature matrix and target
...
# Automatically generated file name by date and time
        ...
        with open(ExportToFile, 'a', newline='\n') as out:
            ...
            if (Flag == False):  # just one time to write the header of the CSV file
                header = numpy.concatenate(
                    [["Optimizer", "Dataset", "objfname", "Experiment", "startTime", "EndTime", "ExecutionTime",
                      "trainAcc", "testAcc", "valAcc"], CnvgHeader1, CnvgHeader1])
                writer.writerow(header)
            a = numpy.concatenate(
                [[x.optimizer, datasets[0], x.objfname, k + 1, x.startTime, x.endTime, x.executionTime, x.trainAcc,
                  x.testAcc, x.valAcc], x.convergence1, x.convergence2])
            writer.writerow(a)
        out.close()
    Flag = True

# Assuming you have your classifier stored in 'x', and you have test data 'X_test' and true labels 'y_test'
# You can obtain predicted probabilities for each class
y_prob = x.predict_proba(X_test)

# For multi-class classification, you need to calculate ROC curve for each class
